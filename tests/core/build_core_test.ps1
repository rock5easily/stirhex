...
# Tests that touch the real clipboard need STIRLING_CORE_TEST_OS=1; without it they are
# skipped so that a plain run never replaces the clipboard contents of whoever runs it.
param([ValidateSet("x86","x64")][string]$Arch = "x64")

$ErrorActionPreference = "Stop"
$root = Split-Path -Parent $MyInvocation.MyCommand.Path
# Keep each architecture in its own subdirectory so an x86 and an x64 build do not
# overwrite each other's exe and object files (Issue #178).
$outDir = Join-Path (Join-Path $root "bin") $Arch
if (-not (Test-Path $outDir)) { New-Item -ItemType Directory -Path $outDir -Force | Out-Null }

$reportDir = Join-Path (Join-Path $root "reports") $Arch
# Remove previous reports before building, so a failed build or crash cannot leave stale success.
foreach ($name in @("report.html", "report.xml")) {
    $reportPath = Join-Path $reportDir $name
...
Write-Host "== run =="
& $exe --report-dir "$reportDir"
$runExit = $LASTEXITCODE
Write-Host "== exit code: $runExit =="
if ((Test-Path -LiteralPath (Join-Path $reportDir "report.xml")) -and
    (Test-Path -LiteralPath (Join-Path $reportDir "report.html"))) {
    Write-Host "Reports: $reportDir"
} else {
    Write-Warning "Core test reports were not generated."
    if ($runExit -eq 0) { $runExit = 2 }
}
exit $runExit
