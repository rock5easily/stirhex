...
- 検索・置換（16 進 / 文字、16 進のワイルドカード `??`）、全て検索と検索結果一覧、不一致検索、複数ファイルを対象とした BGREP 検索
...
- チェックサム・ハッシュ値計算（選択範囲またはファイル全体）
- バイナリパッチ（IPS / BPS）の作成・適用
- 印刷・印刷プレビュー（範囲指定印刷対応）
...
- `tests/core/build_core_test.ps1` — コア機能テストのビルド・実行（`-Arch x64` / `x86`）
- `tests/e2e/` — pytest（uv 管理）による GUI e2e 自動テスト。詳細は `tests/e2e/README.md` を参照
- `tests/static/` — アプリを起動しない静的検証テスト（pytest）
